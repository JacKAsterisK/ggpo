...
SyncTestBackend::LogSaveStates(SavedInfo& info)
{
    ...
    _callbacks.log_game_state(_callbacks.instance, filename, (unsigned char*)info.buf, info.cbuf);

    sprintf_s(filename, ARRAY_SIZE(filename), "synclogs\\state-%04d-replay.log", _sync.GetFrameCount());
    _callbacks.log_game_state(_callbacks.instance, filename, _sync.GetLastSavedFrame().buf, _sync.GetLastSavedFrame().cbuf);
}