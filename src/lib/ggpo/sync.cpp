...
Sync::~Sync()
{
	...
	for (int i = 0; i < ARRAY_SIZE(_savedstate.frames); i++) {
		_callbacks.free_buffer(_callbacks.instance, _savedstate.frames[i].buf);
	}
...
Sync::LoadFrame(int frame)
{
	...
	_callbacks.load_game_state(_callbacks.instance, state->buf, state->cbuf);
...
Sync::SaveCurrentFrame()
{
	...
	SavedFrame* state = _savedstate.frames + _savedstate.head;
	if (state->buf) {
		_callbacks.free_buffer(_callbacks.instance, state->buf);
		state->buf = NULL;
	}
	state->frame = _framecount;
	_callbacks.save_game_state(_callbacks.instance, &state->buf, &state->cbuf, &state->checksum, state->frame);
